[h1]Vanilla Tweaks[/h1]
Vanilla Tweaks is a lightweight mod that fixes base game bugs, adjusts game balance and makes the AI not that extremely stupid.

[h2]Changes[/h2]
[list]
    [*]Rebalanced the Turkish focus tree.
    [*]Made the Defence of the Gulf compatible with GoE.
    [*]Fixed most bugs with formables.
    [*]Added West African formable.
    [*]Added more expansion for Polynesian and Antilles formables.
    [*]Reworked French communism ban.
    [*]All focuses creating a faction now allow countries to make factions.
    [*]Improved performance.
    [*]Fixed Missiolinis Re-Roll.
    [*]Fixed many typos.
[/list]

[h2]Help VT[/h2]
Vanilla Tweaks is a volunteer based project and there is no dev structure. If you want to help out, make a pull request on [url=https://github.com/Xaelorichero-02/Vanilla-Tweaks]GitHub[/url]. Also you can join my [url=https://discord.gg/r9e7ggzGSe]Discord server[/url].

[h2]Contributors[/h2]
[list]
    [*]Xaelor
    [*][url=https://steamcommunity.com/sharedfiles/filedetails/?id=3083531552]Sejozwak[/url]
[/list]

[url=https://discord.gg/r9e7ggzGSe][img]https://i.imgur.com/g7mnaUQ.png[/img][/url]